...
| 5 | `delvec fmt <campaign-dir>` rewrote the six `refimg` sidecars under `design/`, not only the stage documents and l10n sidecars the page says it covers: "examined 15 file(s); reformatted 9... reformatted 6". No data was lost — `request.chain_from` and the resolved frame survive — but a provider record committed beside its image is no longer byte-identical to what the tool wrote, and the page's description of what `fmt` touches does not predict it. | 1 | open |
