"""Minimal Minecraft structure-NBT reader: gzip + big-endian tags."""
import gzip, struct
...
def _rd(f):
    ...
    def payload(t):
        ...
        if t == 6: return f8()
        if t == 7:
            n = i4(); return list(struct.unpack('>%db' % n, f.read(n)))
        if t == 8: return s()
        if t == 9:
            et = u1(); n = i4()
        ...
        if t == 10:
            ...
            while True:
                ...
                d[nm] = payload(ct)
        if t == 11:
            n = i4(); return list(struct.unpack('>%di' % n, f.read(n)))
        if t == 12:
            n = i4(); return list(struct.unpack('>%dq' % n, f.read(n)))
